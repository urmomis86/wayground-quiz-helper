...
If there's no clear question, respond with "NO_QUESTION".`;
  ...
  // Call both AIs simultaneously and get consensus
  async function getBothAIAnswers(question, options) {
    ...
    showStatus('🤖 Calling BOTH AIs at the same time...', 'info');
    ...
    if (keys.openrouter) {
      const openrouterPromise = fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        console.log('OpenRouter parsed number:', num);
        showStatus(`✅ OpenRouter answered: ${num}`, 'success');
        return { source: 'openrouter', answer: num, valid: num >= 1 && num <= options.length };
      }).catch(err => {
    ...
    if (keys.deepseek) {
      const deepseekPromise = fetch('https://api.deepseek.com/chat/completions', {
        ...
        const answer = data.choices?.[0]?.message?.content?.trim();
        console.log('DeepSeek raw answer:', answer);
        const num = parseInt(answer);
        console.log('DeepSeek parsed number:', num);
        showStatus(`✅ DeepSeek answered: ${num}`, 'success');
        return { source: 'deepseek', answer: num, valid: num >= 1 && num <= options.length };
      }).catch(err => {
    ...
    console.log('Waiting for', promises.length, 'AI promises...');
    ...
    if (validAnswers.length === 0) {
      showStatus('❌ Both AIs failed - using fallback option 1', 'error');
      console.log('No valid AI answers, returning fallback 0');
      return 0; // Fallback to first option
    }
    
    // Check if they agree
    const firstAnswer = validAnswers[0].answer;
    const allAgree = validAnswers.every(r => r.answer === firstAnswer);
    
    if (allAgree && validAnswers.length > 1) {
      showStatus('✅ BOTH AIs AGREE on answer!', 'success');
    } else if (validAnswers.length > 1) {
      showStatus(`⚠️ AIs disagree! Using ${validAnswers[0].source}`, 'warning');
    } else {
      showStatus(`✅ Using ${validAnswers[0].source} answer`, 'success');
    }
    
    // Return 0-based index
    return firstAnswer - 1;
  }
...
Respond with just the answer text, nothing else.`;
  ...
  async function processPageWithHybridAI() {
    ...
    // 80% AI / 20% RL decision
    const useRL = Math.random() < 0.2; // 20% chance to use RL model
    const rlModel = selectBestModel();
    ...
    if (useRL) {
      // 20% - Use RL-selected model
      showStatus(`🎯 Using RL model (${rlModel}) - 20% path`, 'info');
      answerIndex = await getAnswerWithRL(questionText, options, rlModel);
      decisionMethod = 'RL_20%';
    } else {
      // 80% - Use AI consensus
      showStatus('🤖 Using AI consensus - 80% path', 'info');
      answerIndex = await getBothAIAnswers(questionText, options);
      decisionMethod = 'AI_80%';
    }
    
    console.log('Answer index:', answerIndex, 'Method:', decisionMethod);
    
    if (answerIndex === null || answerIndex < 0 || answerIndex >= options.length) {
      showStatus('❌ Could not determine answer, trying fallback...', 'error');
      // Fallback to first option
      answerIndex = 0;
      decisionMethod += '_FALLBACK';
    }
