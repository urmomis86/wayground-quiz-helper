...
(function() {
  ...
  function findQuestions() {
    ...
    console.log('[Wayground Debug] Current URL:', window.location.href);
    console.log('[Wayground Debug] Page title:', document.title);
    ...
    console.log('[Wayground Debug] Checking', selectors.length, 'selectors');
    ...
    for (const selector of selectors) {
      ...
      console.log(`[Wayground Debug] Selector "${selector}": ${elements.length} elements`);
      elements.forEach(el => {
        checkedElements++;
  ...
  function findOptions() {
    ...
    for (const selector of selectors) {
      ...
      elements.forEach(el => {
        ...
        console.log(`[Wayground Debug] ✓ Found option (${trimmedText.length} chars, tag=${el.tagName}): "${trimmedText.substring(0, 60)}..."`);
...
Your answer (exact text only):`;
    ...
    console.log('[Wayground Debug] =======================================');
    console.log('[Wayground Debug] OpenRouter key available:', !!keys.openrouter);
    console.log('[Wayground Debug] Cohere key available:', !!keys.cohere);
      ...
      const openrouterPromise = Promise.race([
        (async () => {
          ...
          while (retries > 0) {
            ...
            console.log('[Wayground Debug] OpenRouter raw answer:', answer);
            console.log('[Wayground Debug] =======================================');
...
Respond with just the answer text, nothing else.`;
  ...
  async function processPageWithHybridAI() {
    ...
    console.log('[Wayground Debug] =======================================');
    console.log('[Wayground Debug] CLICKING ANSWER');
    console.log('[Wayground Debug] Answer element:', answerElement);
    console.log('[Wayground Debug] Element tag:', answerElement.tagName);
    console.log('[Wayground Debug] Element type:', answerElement.type);
    console.log('[Wayground Debug] Element id:', answerElement.id);
    console.log('[Wayground Debug] Element classes:', answerElement.className);
    console.log('[Wayground Debug] Element visible:', answerElement.offsetParent !== null);
    console.log('[Wayground Debug] =======================================');
    ...
    if (answerElement.tagName === 'LABEL') {
      console.log('[Wayground Debug] Element is a LABEL, looking for associated input');
      const labelFor = answerElement.getAttribute('for');
      if (labelFor) {
        const input = document.getElementById(labelFor);
        if (input) {
          clickableElement = input;
          console.log('[Wayground Debug] ✓ Found associated input for label:', input);
        } else {
          console.log('[Wayground Debug] ✗ No input found with id:', labelFor);
        }
      } else {
        ...
        if (wrappedInput) {
          ...
          console.log('[Wayground Debug] ✓ Found wrapped input in label:', wrappedInput);
        } else {
          console.log('[Wayground Debug] ✗ No wrapped input found in label');
        }
      }
    }
    ...
    if (answerElement.tagName === 'DIV' || answerElement.tagName === 'SPAN') {
      console.log('[Wayground Debug] Element is a DIV/SPAN, looking for clickable child');
      ...
      if (childInput) {
        ...
        console.log('[Wayground Debug] ✓ Found clickable child element:', childInput);
      } else {
        console.log('[Wayground Debug] ✗ No clickable child found');
      }
    }
    
    console.log('[Wayground Debug] Final clickable element:', clickableElement);
    console.log('[Wayground Debug] Final element tag:', clickableElement.tagName);
    console.log('[Wayground Debug] Final element type:', clickableElement.type);
    
    // Scroll element into view
    console.log('[Wayground Debug] Scrolling element into view');
    clickableElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
    
    // Focus the element
    try {
      console.log('[Wayground Debug] Attempting to focus element');
      clickableElement.focus();
      console.log('[Wayground Debug] ✓ Element focused');
    } catch (e) {
      console.log('[Wayground Debug] ✗ Could not focus element:', e);
    }
    
    // For radio/checkbox inputs, set checked property directly
    if (clickableElement.type === 'radio' || clickableElement.type === 'checkbox') {
      console.log('[Wayground Debug] Element is radio/checkbox, setting checked property');
      clickableElement.checked = true;
      console.log('[Wayground Debug] ✓ Checked property set to:', clickableElement.checked);
      ...
      console.log('[Wayground Debug] ✓ Change/input events dispatched');
    } else {
      // For other elements, use click with multiple methods
      console.log('[Wayground Debug] Using click() method');
      clickableElement.click();
      console.log('[Wayground Debug] ✓ click() called');
      
      // Try alternative click methods
      setTimeout(() => {
        try {
          console.log('[Wayground Debug] Trying MouseEvent dispatch');
          clickableElement.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));
          console.log('[Wayground Debug] ✓ MouseEvent dispatched');
        } catch (e) {
          console.log('[Wayground Debug] ✗ MouseEvent dispatch failed:', e);
        }
      }, 100);
    }
    ...
    setTimeout(() => {
      try {
        console.log('[Wayground Debug] Second click attempt');
        clickableElement.click();
        console.log('[Wayground Debug] ✓ Second click() called');
      } catch (e) {
        console.log('[Wayground Debug] ✗ Second click failed:', e);
      }
    }, 500);
